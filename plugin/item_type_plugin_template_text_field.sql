...
'    else',
'        --',
'        if (not l_is_disabled or not p_param.is_readonly or (l_is_disabled and l_save_state))',
'        then',
'            l_name := apex_plugin.get_input_name_for_item;',
'        end if;',
'        --',
'        sys.htp.prn(',
'            ''<input type="''||l_subtype||''" '' ||',
...
'                                                       || case when l_text_case = ''U''   then '' u-textUpper'' when l_text_case = ''L'' then '' u-textLower'' else '''' end',
'                                                   ) ||',
...
wwv_flow_imp_shared.create_plugin_file(
 p_id=>wwv_flow_imp.id(110638694270734820)
,p_plugin_id=>wwv_flow_imp.id(1459656641314523373)
